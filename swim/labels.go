...
// validateIncomingLabels checks a whole label map (from a peer, or initial
// labels): every key/value within KeySize/ValueSize, at most Count public
// labels, and at most Count internal (__) labels. Internal labels are bounded
// here (unlike validateLabels, which exempts them) so a peer can't bypass the
// limits via the __ namespace.
